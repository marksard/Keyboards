...
| 基板番号 | 定数 | 数 | 名前 | 備考 |
| ---- | ---- | --- | --- | --- |
| C1 | 0.1uF | 2 | コンデンサ(1206size) | プラスチックケースに入っていて1uFよりも色が薄く、厚みもほんのり薄い |
| C2-C3 | 1uF | 3 | コンデンサ(1206size) | 紙色が濃い |
| C4-C5 | 22pF | 3 | コンデンサ(1206size) | 一番色が薄く、厚みもない |
...
| フラックス | 必須 |
| フラックス除去剤 | フラックスを塗るので除去します。ベタベタも解消してキレイになります |
...
　組み立ての時間ですが、2～3時間くらい（ファーム開発環境構築からだと別途+1～2時間程度）を目安にしてください。ハンダの煙は吸ったり目に入れたりしないように注意しましょう。  
...
　写真の向きで取り付けてください。  
　ハンダ付けするピンのピッチがかなり狭く、MicroUSBのケースの内側にピンがあるため難易度は高いです。    
　はんだ付け講座的なYoutube動画などを観てイメージトレーニングしてからやると良いでしょう。  
　隣のピンと半田ブリッジした場合は半田吸い取り線などで除去してください。  
　ルーペやキズミでの目視確認、テスターでの検査の両方で隣のピンと導通していないことを確認してください。  
...
　次にatmega32u4のピンが綺麗に載るように調整します。このとき、基板の銀色のパッドのどこか一辺にフラックスを軽く塗布しておくとちょっとくっつくようになるので位置決めがしやすいと思います。  
　はんだづけはmicro USBと同じ要領でおこなってください。  
　ルーペやキズミでの目視確認、テスターでの検査の両方で隣のピンと導通していないことを確認してください。  
...
　色が薄く一番平たいものは22pFで水晶振動子の両サイドC4,C5の二か所に取り付けます。  

　プラスチックケースに入っていて2個入っているものが0.1uFですので、C1に取り付けます。  
　残ったのは1uFでC2,C3に取り付けます。  
...
　ここまで出来たらもう一度、ルーペやキズミでの目視確認、テスターでの検査の両方で隣のピンと導通していないことmicroUSBとATMEGA32U4を対象に確認してください。  
...
　あとはケースを組み立ててキーキャップを着けるだけですが、ケースを組み立てるのと分解するのが若干面倒なのと、MXソケットやダイオードがキチンと半田付けされているかの動作確認を済ませておくとケース組み立てが一回で済むので「ファームウェアの準備と書き方」を見てファームウェアを書き込んで動作確認をしてください。  
...
　ケースのパーツは2mm厚のものと、5mm厚のものがあります。以下の順番で乗せていきます。  
...
　ボルトを6か所ある穴に差し込み、ナットでボルトを仮止めします。  
　トップレイヤーとキースイッチの間隔に遊びをそれほど作ってないので、キーキャップとトップレイヤーの間隔が均等になるように位置決めしてから、ボルトを締め付けてください。締め付けすぎるとアクリルが割れる可能性もあるので注意してください。  
