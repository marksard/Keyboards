...
　オプションのUnderglowを取り付ける方は、LEDの点灯確認に使いますので必須項目です。  
...
　Treadstone48のデフォルトキーマップのmakeは ```make treadstone48:default``` で可能です。（MSYS2環境上なら ```make treadstone48:default:avrdude``` で書き込みも出来ます）  
...
　Pro Micro直下に取り付けるスペーサーと、OLEDの基板が左右1mm x 3mmくらい干渉しますので、写真のようにカットしてください。
...
　テープLEDの剥離紙の両端数cmだけハサミなどでカットします。テープLEDは向きがありテープLEDの端子と基板の端子が合う向きにします。  
...
　Pro Micro周りの目隠しとOLEDを保護するためのプレートを取り付けます。8mmスペーサー2つを5mmのネジでPCB(写真の黄色〇の箇所)に取り付けます。  
...
　Underglowのテストのためにled_testを書き込んでいた人は戻ってdefaultキーマップを書き込んでください。  
